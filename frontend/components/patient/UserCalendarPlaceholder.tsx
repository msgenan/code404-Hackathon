...
interface UserCalendarPlaceholderProps {
  doctors: string[];
  slots: Slot[];
}
...
const stateStyles: Record<NonNullable<Slot["state"]>, string> = {
  ...
  booked: "bg-white text-slate-500 ring-1 ring-slate-100",
  empty: "bg-slate-50 text-slate-400 ring-1 ring-slate-100",
};

const UserCalendarPlaceholder: React.FC<UserCalendarPlaceholderProps> = ({ doctors, slots }) => {
  return (
    <div className="rounded-3xl bg-white p-4 shadow-xl shadow-sky-50 ring-1 ring-slate-100">
      <div className="flex items-center justify-between pb-4">
        <div>
          ...
          <h3 className="text-xl font-bold text-slate-900">Upcoming Availability</h3>
        </div>
      ...
      <div className="overflow-x-auto">
        <div className="min-w-[720px]">
          <div className="grid grid-cols-[140px_repeat(4,_1fr)] gap-2 text-sm font-semibold text-slate-600">
            <div className="rounded-xl bg-slate-50 px-3 py-2 ring-1 ring-slate-100">Doctors</div>
            {slots.map((slot) => (
          ...
          <div className="mt-2 space-y-2">
            {doctors.map((doctor) => (
              <div key={doctor} className="grid grid-cols-[140px_repeat(4,_1fr)] gap-2 text-sm">
                <div className="flex items-center gap-2 rounded-xl bg-white px-3 py-2 font-semibold text-slate-700 ring-1 ring-slate-100">
                  <span className="inline-flex h-8 w-8 items-center justify-center rounded-xl bg-gradient-to-br from-sky-100 to-emerald-100 text-slate-700 ring-1 ring-slate-100">
                    {doctor.split(" ").map((part) => part[0]).join("")}
                  </span>
                  <div>
                    <p className="leading-tight">{doctor}</p>
                    <p className="text-xs text-slate-500">General / Family</p>
                  </div>
                </div>
                  ...
                  return (
                    ...
                    >
                      <span className="text-center leading-tight">
                        {state === "booked" ? "Booked" : state === "empty" ? "-" : slot.label}
                      </span>
                      <span className="pointer-events-none absolute inset-0 rounded-xl opacity-0 ring-2 ring-emerald-200 transition group-hover:opacity-60" />
                    </div>
                  );
                })}
              </div>
...
export default UserCalendarPlaceholder;
